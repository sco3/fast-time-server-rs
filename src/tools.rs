...
pub fn convert_time(time_str: &str, source_tz: &str, target_tz: &str) -> Result<String, String> {
    let source: Tz = source_tz.parse().map_err(|_| format!("Invalid source timezone: {}", source_tz))?;
    let target: Tz = target_tz.parse().map_err(|_| format!("Invalid target timezone: {}", target_tz))?;
    ...
    let parsed_time = if let Ok(dt) = DateTime::parse_from_rfc3339(time_str) {
        ...
        source.from_local_datetime(&dt).single().ok_or("Ambiguous time")?
...
pub fn handle_tool_call(name: &str, arguments: &serde_json::Value) -> Result<serde_json::Value, String> {
    match name {
        "get_system_time" => {
        ...
        "convert_time" => {
            ...
            let source_tz = arguments.get("source_timezone")
                .and_then(|v| v.as_str())
                .ok_or("source_timezone parameter is required")?;
            let target_tz = arguments.get("target_timezone")
                .and_then(|v| v.as_str())
