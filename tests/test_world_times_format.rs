...
fn test_world_times_format() {
    ...
    for (city, time_value) in times_obj {
        ...
        assert!(time_str.contains('-'), "{} time should contain date separator", city);
        assert!(time_str.contains(':'), "{} time should contain time separator", city);
        assert!(time_str.len() > 10, "{} time should have reasonable length", city);
    }
}