...
fn test_time_query_optional_field() {
    ...
    let query: TimeQuery =
        serde_json::from_str(json_with_null).expect("Should deserialize with null timezone");
    ...
    let query: TimeQuery =
        serde_json::from_str(json_with_value).expect("Should deserialize with timezone");
