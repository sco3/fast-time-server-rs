...
fn test_handle_tool_call_convert_time() {
    ...
    assert!(
        time_text.contains("2025-06-21T12:00:00"),
        "Time should be 12:00 NY time"
    );
    assert!(
        time_text.contains("-04:00"),
        "Should have the correct New York summer offset"
    );
}
