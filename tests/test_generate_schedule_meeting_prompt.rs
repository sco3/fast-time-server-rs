...
fn test_generate_schedule_meeting_prompt() {
    ...
    assert!(prompt.contains("New York"), "Prompt should mention New York");
    ...
    assert!(prompt.contains("60 minutes"), "Should have default duration");
    assert!(prompt.contains("9 AM - 5 PM"), "Should have default preferred hours");
    assert!(prompt.contains("next 7 days"), "Should have default date range");
    ...
    assert!(custom_prompt.contains("90 minutes"), "Should use custom duration");
    assert!(custom_prompt.contains("10 AM - 4 PM"), "Should use custom preferred hours");
    assert!(custom_prompt.contains("next 3 days"), "Should use custom date range");
}