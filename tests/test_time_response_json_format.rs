...
fn test_time_response_json_format() {
    ...
    assert!(json.contains("\"timezone\""), "JSON should contain 'timezone' field");
    assert!(json.contains("\"unix\""), "JSON should contain 'unix' field");
    assert!(json.contains("\"utc\""), "JSON should contain 'utc' field");
    
    // Verify JSON contains expected values
    assert!(json.contains("Europe/London"), "JSON should contain timezone value");
    assert!(json.contains("1776024645"), "JSON should contain unix timestamp");
}