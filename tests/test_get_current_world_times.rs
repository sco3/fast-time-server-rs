...
fn test_get_current_world_times() {
    ...
    let expected_cities = vec!["New York", "Los Angeles", "London", "Paris", "Tokyo", "Sydney", "Dubai"];
    for city in expected_cities {
        assert!(times_obj.contains_key(city), "Should have time for {}", city);
        let time_str = times_obj.get(city).and_then(|v| v.as_str()).expect("City time should be a string");
        assert!(!time_str.is_empty(), "Time for {} should not be empty", city);
    }
}